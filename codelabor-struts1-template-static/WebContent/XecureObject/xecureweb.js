...
/*
 * //증권전산 var accept_cert="SignKorea
 * CA:1.2.410.200004.5.1.1.5:1.2.410.200004.5.1.1.7:1.2.410.200004.5.1.1.9:1.2.410.200004.5.1.1.10:1.2.410.200004.5.1.1.12";
 * 
 * //금융결제원: 상호연동(개인,법인) accept_cert+=
 * ",yessignCA:1.2.410.200005.1.1.1:1.2.410.200005.1.1.5";
 * 
 * //한국정보인증 : 상호연동(개인,법인), 용도제한(증권/보험용) accept_cert+=",signGATE
 * CA:1.2.410.200004.5.2.1.2:1.2.410.200004.5.2.1.1:1.2.410.200004.5.2.1.7.2";
 * 
 * //한국전산원 : 상호연동(개인,법인,기관) accept_cert+=",NCASign
 * CA:1.2.410.200004.5.3.1.9:1.2.410.200004.5.3.1.2:1.2.410.200004.5.3.1.1";
 * 
 ...
 * 
 * //3280 변경된 CA인증서 accept_cert+=",signGATE
 * CA2:1.2.410.200004.5.2.1.2:1.2.410.200004.5.2.1.1:1.2.410.200004.5.2.1.7.2";
 * accept_cert+=",NCASignCA:1.2.410.200004.5.3.1.9:1.2.410.200004.5.3.1.2:1.2.410.200004.5.3.1.1";
 * accept_cert+=",CrossCert Certificate
...
var xgate_addr	= window.location.hostname + ":443:9080";
...
/*
 * document.XecureWeb.SetPolicyConvertTableNext("1.2.410.200005.1.1.2",
 * "금융거래법인"); //
 * document.XecureWeb.SetPolicyConvertTableNext("1.2.410.200005.1.1.3",
 * "금융거래단체");
 * document.XecureWeb.SetPolicyConvertTableNext("1.2.410.200005.1.1.4",
 * "전자거래개인");
 * document.XecureWeb.SetPolicyConvertTableNext("1.2.410.200005.1.1.5",
 * "전자거래법인"); //
 * document.XecureWeb.SetPolicyConvertTableNext("1.2.410.200005.1.1.6",
 * "금융거래단체");
 ...
 * "증권전산"); document.XecureWeb.SetPolicyConvertTableNext("1.2.410.200004.5.3.1",
 * "전산원");
 * document.XecureWeb.SetPolicyConvertTableNext("1.2.410.200004.5.4.1.1",
 * "전자인증"); document.XecureWeb.SetPolicyConvertTableNext("1.2.410.200004.2.1",
 * "공인인증기관");
 */
...
// document.XecureWeb.SetIssuerConvertTableNext("SE CA", "에스이인증기관", "테스트");
	 ...
	 * ""); document.XecureWeb.SetIssuerConvertTableNext("SignGateFTCA",
	 * "정보인증TEST", ""); document.XecureWeb.SetIssuerConvertTableNext("signGate
	 * CA", "정보인증", ""); document.XecureWeb.SetIssuerConvertTableNext("rootca",
	 * "전산원", ""); document.XecureWeb.SetIssuerConvertTableNext("CrossCertCA",
	 * "전자인증", ""); document.XecureWeb.SetIssuerConvertTableNext("CertRSA01",
	 * "KISA인증기관", "");
	 */
...
// var path = "/";
	...
	if( gIsContinue == 0 ) {
		...
		if( IsNetscape60() ){cipher = document.XecureWeb.nsIXecurePluginInstance.BlockEnc(xgate_addr, path, XecureEscape(qs), "GET");}else {
			// cipher = document.XecureWeb.BlockEnc(xgate_addr,
			// "/XecureDemo/jsp/ibs/transfer_input.jsp",
			// XecureEscape(qs),"GET");
...
function XecureMakePlain_Org(form)
{
	...
	for (i = 0; i < len; i++) {
		if (form.elements[i].type != "button" && form.elements[i].type != "reset" && form.elements[i].type != "submit") {
			if (form.elements[i].type == "radio" || form.elements[i].type == "checkbox") { // Leejh
																							// 99.11.10
																							// checkbox추가
...
function XecureMakePlain(form)	// modified by tiger on 2004/12/22
{
       ...
       len = form.elements.length;
       for (i = 0; i < len; i++) {
            // if (form.elements[i].type != "button" && form.elements[i].type !=
			// "reset" && form.elements[i].type != "submit") {
            if (form.elements[i].type != "button" && form.elements[i].type != "reset") {            	
                                 ...
                                 // for softcamp
                                              ...
                                              if (form.elements[i].type == "select-one") {
                                                ...
                                                if (op_len > 0) {
                                                          if(ind > 0) {
                                                                     if (form.elements[i].options[ind].value != ''){value[j] = form.elements[i].options[ind].value;} else {// value[j]
																																											// =
                                                ...
                                                for( k = 0; k < llen; k++) {
                                                          if (form.elements[i].options[k].selected) {
                                                                     name[j] = form.elements[i].name;
                                                                     if (form.elements[i].options[k].value != ''){value[j] = form.elements[i].options[k].value;} else {// value[j]
																																										// =
																																										// form.elements[i].options[k].text;
																	   value[j] = "";}
                                                                     j++;
                                                                     increased++;
                                                          }
...
// nOption is 0 : (default value) File version, which is checked by 'Internet
// Explorer'
// 1 : Product version
// 2 : File Description
...
function UpdateModules( infoURL )
{
	...
	{
		// success : 0, cancel : 1, file(s) holded : 2, already updated : 3,
		// invalid user : 4, need not : 5
		// error : -1
 ...
 * 
 * pos = 0; j = -2; k = -2; while (pos > -1) { if(j == -2) j = url.indexOf('+',
 * pos); if(k == -2) k = url.indexOf('=', pos); if(j < 0 && k < 0) break; if ((j <
 * k && j > -1) || (j > -1 && k < 0)) { out += url.substring(pos, j); out +=
 * '%2B'; pos = j + 1; j = -2; } else if ((j > k && k > -1) || (k > -1 && j <
 * 0)) { out += url.substring(pos, k); out += '%3D'; pos = k + 1; k = -2; } else
 * pos = -1; } return out; }
 */
 ...
 * 
 * It is possible to combine following option flags [EXCEPTION] - 1,2 cannot be
 * used simultaneously - 4 is valid for only 1
 * 
 * envOption = 1 : 인증서기반 전자봉투 = 2 : 패스워드기반 전자봉투 = 4 : 여러 개의 인증서로 전자봉투 = 8 : CMS
 * 타입으로 Envelop = 256 : 로그인한 인증서로 전자봉투
 * 
...
function make_html(s_name,s_value) {
    ...
    MsgNm = "<body topmargin='0' leftmargin='0' style='overflow-x:hidden;overflow-y:hidden'>"
                               ...
                               + "      <img src='http://img.kbstar.com/common/login/top.gif' align='absmiddle'> <b>"+" 아래의 내용을 확인하신 후 서명해 주십시오</b>"
                               + "    </td>"
                               + "  </tr>"
                               + "  <tr>"
    ...
    MsgNm = MsgNm
                               +"        <tr><td width='100'><img src='http://img.kbstar.com/common/bullet/bu_32.gif' border='0'> <b>"+intMsg[i]+"</b></td>"
                               + "            <td>: [%s]</td></tr>";
    }
    MsgNm = MsgNm
                               + "      </table>"
...
{
	var signed_msg;
// XW_SIGN_OPTION_VID_OPT_NULL_IDN 16
// option + 16
...
function Sign_html(option, s_name, s_value, site ) {
    var html_tpl = make_html(s_name, s_value);
    if (html_tpl == "" ){
      alert(" 서명할 데이터 개수가 서로 맞지 않습니다. ");
      return "";
    }
    var signed_msg = Sign_with_option_htmlex(option, html_tpl, s_value, site);
    return signed_msg;
}

function XecureSubmit_noEnc( form )
{
	var qs ;
	var path ;
	var cipher;
	...
	if ( form.method == "get" || form.method=="GET" ) {
		...
		if( gIsContinue == 0 ) {
			gIsContinue = 1;
			if( IsNetscape60() ){sid = document.XecureWeb.nsIXecurePluginInstance.BlockEnc(xgate_addr, path, XecureEscape(qs),"GET");}else {
		...
		document.xecure.action = path + "?q=" + sid + XecureAddQuery(qs);
