 ...
 * 
 */
public class SimplePatternReplaceUtils {
	/**
	 * 로거
	 */
	static private Logger logger = LoggerFactory.getLogger(SimplePatternReplaceUtils.class);
	...
	static public String replace(Map<String, String> searchAndReplacePatternMap, List<String> excludesPatternList, String targetString) {
		...
		if (searchAndReplacePatternMap != null) {
			if (excludesPatternList != null && SimplePatternMatchUtils.matches(excludesPatternList, targetString)) {
				...
				while (iter.hasNext()) {
					...
					if (SimplePatternMatchUtils.matches(searchPattern, targetString)) {
						replacePattern = searchAndReplacePatternMap.get(searchPattern);
