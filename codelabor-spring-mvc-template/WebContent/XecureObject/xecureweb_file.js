...
function FileUpload(link)
{
	...
	if(extname == 'php'){docKind = "php";} else {docKind	= "jsp";}
	...
	if(navigator.appName == 'Netscape') {
    	...
    	if(agent != 'Mozilla/5') {
			...
			if(docKind == "php"){r = document.FileAccess.FileUpload(
				xgate_addr, "/cgi-bin/xw_upload.cgi", "", hostname, port, filename, escape(desc_upload));} else {r = document.FileAccess.FileUpload(
		...
		if(docKind == "php"){r = document.FileAccess.FileUpload(
				...
				hostname, port, filename, desc_upload);} else {r = document.FileAccess.FileUpload(
...
function FileUploadEx( filePath, link, upOption )
{
	...
	if(navigator.appName == 'Netscape') {
    	...
    	if(agent != 'Mozilla/5') {
      	...
      	path = "/" + path;
      		if( filePath == "" ){filename = document.FileAccess.FileSelect2(initdir_up);} else {filename = filePath;}
		...
		if(docKind == "php"){r = document.FileAccess.FileUploadEx(
				xgate_addr, "/cgi-bin/xw_upload.cgi" ,"", 
				hostname, port, filename, desc_upload, upOption);} else {r = document.FileAccess.FileUploadEx(
...
*/
	...
	if( r == "CANCEL" ){alert('암호화 파일 다운로드가 취소되었습니다.');}else if( r != "" )
	{
...
// var signOption = 4 + 8 + 32 + 64;
	...
	{	
		if( filePath == "" || filePath == null){filename = document.FileAccess.FileSelect2(initdir_up);} else {filename = filePath;}
...
function FileClear( xaddr )
{
	...
	{
		alert( "Not supported function" );
	} else {document.FileAccess.FileClear( xaddr, 0 );}
...
// var zipOption = 32;
	...
	{	
		if( filePath == "" ){filename = document.FileAccess.FileSelectEx(initdir_up, selOption);} else {filename = filePath;}
...
// var uzOption = 0;
	...
	{	
		if( filePath == "" ){filename = document.FileAccess.FileSelectEx(initdir_up, 0);} else {filename = filePath;}
...
function MultiFileUpload(link)
{
    	...
    	else if( filename == "CANCEL" )
	{
    	...
    	else
    	{
    		alert(filename);
    		path = "/" + path;
    		r = document.FileAccess.MultiFileUpload(
					xgate_addr, path ,qs, hostname, port, filename, desc_upload, 256 );
...
function MultiFileDownload(fileDir, link)
{
	...
	response = document.FileAccess.MultiFileDownload( xgate_addr, path, qs, hostname, port, fileDir, "", "", 256);		
